---
title: 虚拟列表-大数据渲染的解决方案
date: 2022-06-01 21:34:56
tags:
  - 虚拟列表
categories:
  - 前端
...
要想实现虚拟列表功能，首先需要一个固定可视区域去展示数据（数据展示区域），并且还要有一个区域去模拟所有数据所占的空间（滚动区域）。
当滚轮滚动时，获取到此时的 scrollTop 值，然后根据这个值去计算应当显示的数据是哪几条，再将这些数据渲染至数据展示区域。
如此以来，网页只会显示用户应该看到的数据。
